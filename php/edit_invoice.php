...
//getting the quotation number
	...
	$quotation_id = $query_result['id'];
	$date = $query_result['date'];
	
	$customer = $query_result['customer'];
	$customer_company = $query_result['customer_company'];
...
<!-----choose payment date form------->
			...
			<tbody>
				...
				<?php
					...
					while ($query_assoc = mysqli_fetch_assoc($get_more_info_query_run))
					{
						...
						$discount_amount = $discount*$quantity*$rate/100;

						$cgst = $query_assoc['cgst'];
						$cgst_amount = ($rate*$quantity - $discount_amount)*$cgst/100;

						$sgst = $query_assoc['sgst'];
						$sgst_amount = ($rate*$quantity - $discount_amount)*$sgst/100;

						$igst = $query_assoc['igst'];	
						$igst_amount = ($rate*$quantity - $discount_amount)*$igst/100;
	...
	<!-------payment method and payment date----------->
		...
		<input type="button" style="display: none;" value="Generate Invoice" id="invoice_gen_edit_button">
	</div>
...
<!---------script----------->
		...
		//creating a new id in database to store this row
			$.post('php/create_new_quotation_row_db.php', {quotation_customer: quotation_customer, quotation_date: quotation_date, quotation_num:quotation_num, quotation_brand:quotation_brand, quotation_model_name:quotation_model_name, quotation_model_number:quotation_model_number, quotation_serial_num:quotation_serial_num, quotation_service_id:quotation_service_id, type:type}, function(e)
			{
				if(e != 0)
				{
					var new_id = e;
					var way = "edit";
					$.post('php/add_new_invoice_item.php', {new_id: new_id, way:way},function(data)
					{
	...
	//on clicking on save edit button
		...
		//getting variable values
			...
			var payment_method = $.trim($('#payment_method1').val());
			var payment_date = $.trim($('#payment_date1').val());	
			...
			//editing invoice if availability is ok												
			    if(to_display_button == 0) //good to generate invoice
			    {
					...
					//defining variables of each table row
						...
						var quotation_serial_num = $.trim($('.quotation_entry_table tr:nth-child('+ child_no + ') #quotation_serial_num').val());
					...
					//adding this to database	
						...
						$.post('php/query_runner.php', {query_recieved:query_recieved}, function(e)
						{
							...
							{
								$('.gen_quotation_span').text('Something went wrong while editing the invoice').css('color','red');
							}
						});
					}
			    }
			    ...
			    {
			    	//$('#invoice_gen_edit_button').fadeOut(0);
			    	$('.gen_quotation_span').text("You have entered quantity greater than its avavilability in stock. You are not able to generate invoice.").css('color', 'red');
			    }
			}
			else
			{
