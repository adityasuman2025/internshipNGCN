...
<div class="inventory_tab">
	<button class="service_quot_list_button">Service Invoices</button>
	<button class="sales_quot_list_button">Sales Invoices</button>
</div>
<br><br>
...
<!---script------>
	...
	//by default service quotation are shown
		$('.quotation_list_container h3').text('Service Invoices');
		...
		var query = "SELECT * FROM quotation WHERE type='service' AND payment_method != '' AND creator_branch_code = '" + creator_branch_code + "' GROUP BY quotation_num ORDER BY date_of_payment DESC";
		
		$.post('php/list_invoice.php', {query:query}, function(data)
		{
	...
	//switching tab b/w service quotation and sales quotation
		...
		{
			$('.quotation_list_container h3').text('Sales Invoices');
			$('.quotation_list_container table').html("<img class=\"gif_loader\" src=\"img/loaders1.gif\">");

			var creator_branch_code = "<?php echo $creator_branch_code; ?>";
			var query = "SELECT * FROM quotation WHERE type='sales' AND payment_method != '' AND creator_branch_code = '" + creator_branch_code + "' GROUP BY quotation_num ORDER BY date_of_payment DESC";
